...
class Ticket {
  ...
  async createTicketInicio(corpo_texto, data_limite, sub, doc1, doc2, eProfessor){
    try{
      ...
      var id_ticket = await knex.select(['id']).table('ticket').where({feedback: null, id: id.id})
      await knex.insert({ id_ticket: id_ticket.id_ticket, arquivo: doc1, tipo: "TCE", eProfessor: eProfessor}).table("documento");
      await knex.insert({ id_ticket: id_ticket.id_ticket, arquivo: doc2, tipo: "PA", eProfessor: eProfessor}).table("documento");
      return true;  
    } catch(error){
      console.log(error);
      return false;
